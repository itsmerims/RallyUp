...
export default function LiveSessionView() {
  ...
  useEffect(() => {
    ...
    firestoreService.getSessionMapping(sessionCode).then(mapping => {
      ...
      unsubscribers = [
        firestoreService.subscribeToPlayers(mapping.qmUserId, setPlayers, sessionId),
        firestoreService.subscribeToCourts(mapping.qmUserId, setCourts),
        firestoreService.subscribeToMatches(mapping.qmUserId, setMatches, sessionId),
      ];
      setStatus('live');
    });
  ...
  return <div className="panel-scrollbar min-h-screen overflow-y-auto bg-slate-950 text-slate-100">
    <header className="sticky top-0 z-20 flex min-h-20 items-center justify-between border-b border-slate-800 bg-slate-950/90 px-4 py-4 backdrop-blur-xl md:px-8">
      ...
      <div className="flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-2"><span className="relative flex h-2.5 w-2.5"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-70" /><span className="relative h-2.5 w-2.5 rounded-full bg-emerald-400 shadow-[0_0_10px_#34d399]" /></span><span className="hidden text-[10px] font-black uppercase tracking-wider text-emerald-300 sm:inline">Live Realtime Tracking</span><span className="text-[10px] font-black text-emerald-300 sm:hidden">LIVE</span></div>
    </header>
    ...
    <footer className="border-t border-slate-800 px-4 py-8 text-center"><p className="text-[10px] text-slate-500">This dashboard updates automatically in real-time. Players do not need to refresh this page.</p><p className="mt-2 text-[10px] text-slate-600">Built with&nbsp; by <a href="https://github.com/itsmerims" target="_blank" rel="noreferrer" className="font-bold text-slate-400 hover:text-emerald-400">Rims</a></p></footer>
  </div>;
}
