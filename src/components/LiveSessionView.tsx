...
export default function LiveSessionView() {
  ...
  return <div className="panel-scrollbar h-screen overflow-y-auto bg-slate-950 text-slate-100">
    <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-2 border-b border-slate-800 bg-slate-950/90 px-3 py-3 backdrop-blur-xl sm:min-h-20 sm:px-4 sm:py-4 md:px-8">
      <div className="flex min-w-0 items-center gap-2 sm:gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-500 shadow-[0_0_22px_rgba(239,68,68,.3)] sm:h-11 sm:w-11"><span className="text-[8px] font-black text-white sm:text-xs">LOGO</span></div><div className="min-w-0"><h1 className="truncate text-xs font-black uppercase tracking-tight text-white sm:text-base md:text-xl">RallyUp Badminton App</h1><p className="truncate text-[8px] font-bold uppercase tracking-[.15em] text-slate-500 sm:text-[9px] sm:tracking-[.22em]">Live Session View · {sessionCode}</p></div></div>
    ...
    <main className="mx-auto max-w-[1600px] space-y-5 p-4 md:p-6 lg:p-8">
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">{metrics.map(metric => <div key={metric.label} className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-900/55 p-4 shadow-xl shadow-black/10 lg:gap-4 lg:p-5"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-slate-950 text-emerald-400 lg:h-10 lg:w-10"><metric.icon className="h-5 w-5" /></div><div className="min-w-0"><div className="truncate text-lg font-black tracking-tight text-white lg:text-2xl">{metric.value}</div><div className="mt-0.5 truncate text-[8px] font-bold uppercase tracking-wider text-slate-500 lg:text-[9px] lg:tracking-widest">{metric.label}</div></div></div>)}</section>
